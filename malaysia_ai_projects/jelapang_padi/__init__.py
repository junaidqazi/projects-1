...
from malaysia_ai_projects.utils import imagenet_input
...
repo_id = 'malay-huggingface/jelapang-padi'
...
def load(model: str = 'efficientnet-b2', **kwargs):
    ...
    if model not in _available_models:
        raise ValueError(
            'model not supported, please check supported models from `malaysia_ai_projects.jelapang_padi.available_model()`.'
        )
...
class Model:
    ...
    def predict(self, inputs: List[np.array]):
        ...
        for input in inputs:
            ...
            r = resize(input, (self._size, self._size), anti_aliasing=False)
            r = imagenet_input(r)
            batch.append(r)
        r = self._execute(
            inputs=[batch],
